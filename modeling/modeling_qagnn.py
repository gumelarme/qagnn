...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        # MAYBE: this seems to flatten the inputs
        # and then rejoining it all together
        _inputs = (
                # batch_qids, batch_labels
                [x.view(x.size(0) * x.size(1), *x.size()[2:]) for x in inputs[:-6]]

                # batch_tensor0, batch_list0,  batch_tensor1, batch_list1
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        ...
        if not detail:
            ...
            return logits, attn, concept_ids.view(batch_size, num_choice, -1), node_type_ids.view(batch_size,
                                                                                                  num_choice,
